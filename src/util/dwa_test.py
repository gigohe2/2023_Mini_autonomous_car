...
class Config:
    ...
    def __init__(self):
        ...
        # obstacles [x(m) y(m), ....]
        rospy.Subscriber('/scan_xy', scan_xy, self.cb_scan)
        self.ob = None
        rospy.wait_for_message('/scan_xy', scan_xy)
        

    def cb_scan(self, data):
        self.obs = [data.x, data.y]
        self.ob = np.array([data.x, data.y]).T
        #print("cb obs", np.shape(self.ob))
...
def main(gx=10.0, gy=10.0, robot_type=RobotType.circle):
    ...
    while True:
        ob = config.ob
        idx = np.where( (ob[:,0]<1.5) & (ob[:,0] > -1.5) & (ob[:,1] > 0) & (ob[:,1] < 5))
        ob = np.squeeze(ob[idx, :], 0)
